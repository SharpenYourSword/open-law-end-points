...
class Syncronizr
  ...
  def is_repo_clean?
    status = `git status -s`.each_line.map{|l| l.split(' ')}
  ...
  def dirty_yamls_and_dirty_jasons(files)
    @to_change = files.reduce({:yaml=>[],:json=>[]}) do |hash, file|
      hash[:yaml] << file.pop if File.extname(file) == ".yaml"
      hash[:json] << file.pop if File.extname(file) == ".json"
      hash
    end
    unless @origin then find_origin_master end
    make_dirty_yamls_into_clean_jasons(@to_change[:yaml].uniq!)
    make_dirty_jasons_into_clean_yamls(@to_change[:json].uniq!)
  end
  ...
  def make_dirty_yamls_into_clean_jasons(*files)
    unless files.empty?
      files.each do |yaml_file|
        ...
        json_file = File.basename(yaml_file, ".yaml") + ".json"
        IO.write(JSON.dump(json_file, data))
      end
    end
  end
  ...
  def make_dirty_jasons_into_clean_yamls(*files)
    unless files.empty?
      files.each do |json_file|
        ...
        yaml_file = File.basename(json_file, ".json") + ".yaml"
        IO.write(YAML.dump(yaml_file, data))
      end
    end
  end
  ...
  def organize_the_data_set(data)
    ...
    ordered_data.reduce({}){ | hash, key | hash[key] = data[key] }
  end
end